...
export class SkeletonKey {
	...
	#openPorts(target) {
		...
		for (let key of availableKeys)
			switch (key) {
				case this.#KEYS[0]: this.#ns.brutessh(target);  break;
				case this.#KEYS[1]: this.#ns.ftpcrack(target);  break;
				case this.#KEYS[2]: this.#ns.relaysmtp(target); break;
				case this.#KEYS[3]: this.#ns.httpworm(target);  break;
				case this.#KEYS[4]: this.#ns.sqlinject(target); break;
				default: Log.error(this.#ns, `SKELETON_KEY - Key ${key} doesn't exists.`);
			}
	}
