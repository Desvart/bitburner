...
import {INs, ptimestamp} from '/pkg.helpers';
...
export async function main(ns: INs) {
    ...
    await runner.upload(['pkg.hk.js', 'pkg.gw.js', 'pkg.wk.js', 'pkg.helpers.js', '/services/service.js']);
    
    
    const batchStopTime: ptimestamp = performance.now() + target.wk.duration + 500;
    ...
    */
    ...
    ns.print({hThreads});
    if (hThreads > 0)
        ns.exec('pkg.hk.js', runner.id, hThreads, target.id, batchStopTime - 150,
    ...
    */
    ...
    // const secLevel = 2;
    let weakThreads1: number = target.wk.getThreadsAmount(secLevel, runner.cores);
    ...
    if (w1Threads > 0)
        ns.exec('pkg.wk.js', runner.id, w1Threads, target.id, batchStopTime - 100,
            JSON.stringify({runner: runner.id, threads: w1Threads}));
    
    const growThreads: number = target.gw.getThreadsAmount(Math.min(target.money.available, target.money.max - stolenMoney), target.money.max, runner.cores);
    const gThreads = Math.min(growThreads, runner.getThreadsCount(1.75));
    // const gThreads = 0;
    ns.print({gThreads});
    if (gThreads > 0) {
        ns.exec('pkg.gw.js', runner.id, gThreads, target.id, batchStopTime - 50,
            JSON.stringify({runner: runner.id, threads: gThreads}));
        
        const gSecIncrease: number = target.gw.getSecurityIncrease(gThreads, runner.cores);
        let w2Threads: number = target.wk.getThreadsAmount(gSecIncrease, runner.cores);
        w2Threads = Math.ceil(gThreads * 32 /  (25 * (runner.cores + 15)));
        ns.print(`weaken1Threads: ${w2Threads}`);
        ns.exec('pkg.wk.js', runner.id, w2Threads, target.id, batchStopTime,
